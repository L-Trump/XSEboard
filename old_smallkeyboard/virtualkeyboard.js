    ...
    var d = function(i) {
        ...
        case "input_method":
            if (document.getElementById("kb_langselector").value == 'US US') {
                $("#kb_binput_method a").css("background-position", "-680px -171px");
...
function() {
    ...
    var e = function() {
        var x = ['<table>'];
        for (var X = 0,
        z = Math.ceil(_.length / 10); X < z; X++) {
            if (Q || X == O) {
                x.push('<tr>');
                for (var Z = 0,
                w = X * 10; Z < 10 && !isUndefined(_[w + Z]); Z++) {
                    x.push("<td><a href=''>");
                    if (X == O) {
                        x.push("<b>&nbsp;" + ((Z + 1) % 10) + ": </b>");
                    } else {
